...
describe('Tweet', () => {
  it('should construct', () => {
    assert.doesNotThrow(() => { createTweet('123') })
  })

  it('should only accept strings', () => {
    assert.throws(() => { createTweet(123) })
  })
  ...
  describe('User', () => {
    it('should provide a getter for the profile image url', async () => {
      const t = createTweet(Tweets.text)
      await t.retrieve()
      assert.equal(t.profileImageUrl, 'http://pbs.twimg.com/profile_images/674199882773737473/HqVe72sQ_bigger.png')
    })
  ...
  describe('Media', () => {
    it('getImageUrl() defaults to first image', async () => {
      ...
      assert.equal(url, 'http://pbs.twimg.com/tweet_video_thumb/DMBr2ePW0AASEbV.jpg')
  ...
  describe('Link', () => {
    it('should retrieve information from the linked page', async () => {
      ...
      assert.equal(link.url, 'https://www.heise.de/newsticker/meldung/Flache-Erde-Tueftler-verschiebt-Raketenflug-3901531.html')
      assert.equal(link.image, 'https://www.heise.de/imgs/18/2/3/2/5/0/9/6/rocket-38b7eefe16172f7b-407f5fd2cdedfec3.jpeg')
      assert.equal(link.description, 'Der Beweis, dass die Erde flach statt rund ist, lässt weiter auf sich warten: US-Tüftler Mike Hughes hat keine Genehmigung für den Start seiner selbstgebauten…')
      assert.equal(link.title, '"Flache Erde"-Tüftler verschiebt Raketenflug |\n    heise online\n')
    })
    ...
    it('should should not throw an exception if a link image does not exist', async () => {
      const t = createTweet(Tweets.text)
      await t.retrieve()
      assert.equal(await t.getLinkImage(), undefined)
    })
  })
})
