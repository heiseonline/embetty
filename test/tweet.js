...
describe('Tweet', () => {
  ...
  describe('User', () => {
    it('should provide a getter for the profile image url', async () => {
      const t = createTweet(Tweets.text)
      await t.retrieve()
      assert.strictEqual(t.profileImageUrl, 'http://pbs.twimg.com/profile_images/674199882773737473/HqVe72sQ_bigger.png')
    })
  ...
  describe('Media', () => {
    it('getImageUrl() defaults to first image', async () => {
      ...
      assert.strictEqual(url, 'http://pbs.twimg.com/tweet_video_thumb/DMBr2ePW0AASEbV.jpg')
  ...
  describe('Link', () => {
    it('should retrieve information from the linked page', async () => {
      ...
      assert.strictEqual(link.url, 'https://www.heise.de/newsticker/meldung/Flache-Erde-Tueftler-verschiebt-Raketenflug-3901531.html')
      assert.strictEqual(link.image, 'https://www.heise.de/imgs/18/2/3/2/5/0/9/6/rocket-38b7eefe16172f7b-407f5fd2cdedfec3.jpeg')
      assert.strictEqual(link.description, 'Der Beweis, dass die Erde flach statt rund ist, lässt weiter auf sich warten: US-Tüftler Mike Hughes hat keine Genehmigung für den Start seiner selbstgebauten…')
      assert.strictEqual(link.title, '"Flache Erde"-Tüftler verschiebt Raketenflug |\n    heise online\n')
    })
